    ...
    <div className="relative w-full aspect-square">
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
          ...
          <h3 className="text-black text-lg font-bold mb-2 line-clamp-2 drop-shadow-lg px-2 py-1 inline-block" style={{ backgroundColor: '#f6bf0c' }}>
            {title}
          </h3>

          {/* Description with highlight background */}
          {description && (
            <p className="text-black text-sm mb-3 line-clamp-2 drop-shadow-md px-2 py-1 inline-block" style={{ backgroundColor: '#f6bf0c' }}>
              {description}
            </p>
          )}

          {/* Meta Information with highlight background */}
          <div className="flex items-center justify-between text-sm">
            {author && (
              <div className="flex items-center space-x-2 rtl:space-x-reverse px-2 py-1 inline-block" style={{ backgroundColor: '#f6bf0c' }}>
                <div className="w-6 h-6 bg-black/20 backdrop-blur-sm rounded-full flex items-center justify-center border border-black/40">
                  <span className="text-xs font-medium text-black">
                    {author.charAt(0).toUpperCase()}
                  </span>
                </div>
                <span className="text-black text-xs drop-shadow-md">{author}</span>
              </div>
            )}
            
            {date && (
              <span className="text-black text-xs drop-shadow-md bg-black/10 px-2 py-1 backdrop-blur-sm inline-block" style={{ backgroundColor: '#f6bf0c' }}>
                {date}
              </span>
            )}
          </div>
