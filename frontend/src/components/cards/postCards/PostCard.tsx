...
export default function PostCard({
  ...
  useEffect(() => {
    if (isNew && cardRef.current) {
      const updateBadgePosition = () => {
        ...
        if (rect) {
          setBadgePosition({
            top: rect.top + window.scrollY - 8, // -8px for overflow
            right: window.innerWidth - rect.right - 8 // -8px for overflow
          });
        }
      };
  ...
  return (
    <>
      <div 
        ref={cardRef}
        className="relative w-full aspect-square rounded-2xl overflow-hidden cursor-pointer group shadow-lg hover:shadow-xl transition-all duration-300"
        onClick={onClick}
      >
        {/* Background Image */}
        <div 
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 z-10">
          {/* Title */}
          <h3 className="text-white text-lg font-bold mb-2 line-clamp-2 drop-shadow-lg group-hover:text-yellow-200 transition-colors duration-200">
            {title}
          </h3>
        ...
        <div className="absolute inset-0 border-2 border-transparent group-hover:border-white/30 rounded-2xl transition-all duration-300 pointer-events-none" />
        
        {/* Additional hover effect */}
        <div className="absolute inset-0 bg-black/0 group-hover:bg-black/10 transition-all duration-300 rounded-2xl" />
      </div>

      {/* NEW Badge - Fixed positioning to overflow */}
      {isNew && (
        <motion.div
        ...
        >
          ...
          >
            ...
            <text 
              ...
              y="10" 
              textAnchor="middle" 
              className="text-[6px] font-extrabold fill-black"
