...
interface WilayahImagesContainerProps {
  images: WilayahImage[];
  wilayahName?: string;
}

export default function WilayahImagesContainer({ 
  images, 
  wilayahName = "الولاية" 
}: WilayahImagesContainerProps) {
  const t = useTranslations('wilayah.images');
  const [showModal, setShowModal] = useState(false);
  const [imageErrors, setImageErrors] = useState<{[key: string]: boolean}>({});

  console.log('WilayahImagesContainer - received images:', images);
  ...
  console.log('WilayahImagesContainer - sorted images:', sortedImages);
    ...
    return (
      <div className={`${className} relative rounded-2xl overflow-hidden cursor-pointer group`} onClick={onClick}>
        <Image
          ...
          unoptimized={imageUrl.startsWith('http://127.0.0.1:9000')}
        />
      </div>
    );
  };
