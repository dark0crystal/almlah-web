...
export default function RestaurantsModal({
  ...
  const text = {
    ...
    error: locale === 'ar' ? 'خطأ في تحميل المطاعم' : 'Error loading restaurants',
    tryAgain: locale === 'ar' ? 'حاول مرة أخرى' : 'Try Again',
    noRestaurants: locale === 'ar' ? 'لم يتم العثور على مطاعم' : 'No restaurants found',
    featuredRestaurants: locale === 'ar' ? 'المطاعم المميزة' : 'Featured Restaurants',
    restaurants: locale === 'ar' ? 'المطاعم' : 'Restaurants',
  ...
  useEffect(() => {
    const loadRestaurants = async () => {
      if (!restaurantCategoryId) {
        setError('Invalid restaurant category');
        setLoading(false);
        return;
      }
    ...
    loadRestaurants();
  }, [restaurantCategoryId, selectedGovernateId, text.error]);
      ...
      <div className={`relative bg-white rounded-t-3xl shadow-2xl w-full max-w-4xl transition-all duration-500 ease-out transform ${
        ...
        <div className={`flex items-center justify-between p-6 border-b border-gray-100 ${
          ...
          <div className={`flex items-center space-x-3 ${locale === 'ar' ? 'space-x-reverse' : ''}`}>
            ...
            <h2 className="text-2xl font-bold text-gray-800">{text.restaurants}</h2>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex justify-center items-center min-h-[300px]">
              <div className="text-center text-gray-500">
                ...
                <div className="text-lg">{text.noRestaurants}</div>
              </div>
            </div>
          ) : (
